...
struct Furniture
{
    ...
    double cost;
    /**
    Algorithm to sort the struct by cost
    **/
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Non-member function!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
**/
bool compare_furniture(Furniture a, Furniture b) {
...
void printHeading( string title, int width, ostream& os );
...
void printHeading( string title, int width, ostream& os = cout )
{
    //Declare Variables
    int magic_width = 0;
    ...
    os << "\n";
    os << left  << setfill('=') << setw( width ) << "=" << "\n";
    os << right << setfill(' ') << setw( magic_width ) << title << "\n";
    os << right << setfill('=') << setw( width ) << "=" << endl;
...
void getFurnitureData(ifstream& inFile)
{
    ...
    **/
    std::sort(furniture, furniture + totalFurniture, compare_furniture);
...
void displayFurniture(Furniture furniture[], int totalFurniture)
{
    ...
    /**
    Set the output file type, move the print header so that it prints into the output.txt, duplicate the outputs so it shows on command line and output.txt
    (Made output overwrite existing file since instructions were not clear)
    **/

    ofstream output("SequencedOrders.txt");

    printHeading("Furniture", 60, output);
    ...
    for(int i = 0; i < totalFurniture; i++)
    {
        output << left << setw(13) << furniture[i].name << "   | "
...
void displayFabric(Fabric fabric[], int totalFabric)
{
    ...
    **/
    ...
    for(int i = 0; i < totalFabric; i++)
    {
        output << left << setw(15) << setfill(' ') << fabric[i].name << " | "
...
void displayArt(Art art[], int totalArt)
{
    ...
    **/
    ...
    for(int i = 0; i < totalArt; i++)
    {
        output << left  << setw(18) << art[i].name << "   |   "
...
void displayBook(Book book[], int totalBook)
{
    ...
    **/
    ...
    for(int i = 0; i < totalBook; i++)
    {
        output << left  << setw(25) << book[i].name << "   |  "
